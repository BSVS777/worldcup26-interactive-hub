// Optional compatibility probe: the deployed API allows public GET access
// (see tools/live-api-probe.mjs), so this authenticated path is not required
// to load data. It stays only in case /auth/authenticate is ever exercised
// against a real account for the academic JWT requirement.
...
async function main() {
  ...
  console.log(`LIVE_API_AUTH_PROBE_PASS auth=200 token=present stadiums=${counts.stadiums} games=${counts.games} teams=${counts.teams} groups=${counts.groups}`);
}
