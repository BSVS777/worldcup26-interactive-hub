...
| API-001 | Endpoint real de autenticación verificado | `tools/live-api-auth-probe.mjs`, `js/config.js:AUTH_ENDPOINT` | Probe autenticado opcional preparado; sin credenciales imprime `LIVE_API_AUTH_PROBE_SKIPPED`; requiere `LIVE_API_AUTH_PROBE_PASS` para verificar | EN PROGRESO | BLOQUEO POR CONTRADICCION EXTERNA: la API publica desplegada no exige este flujo para lectura (`npm run test:live-api` ya lee `stadiums`/`games`/`teams`/`groups` sin token); verificacion de `/auth/authenticate` en si sigue pendiente de credenciales validas y de decision del profesor sobre si el requisito de Bearer sigue aplicando. |
| API-002 | Payload de autenticación verificado | `tools/live-api-auth-probe.mjs`, `js/api.js:authenticate` | Probe envia `{ email, password }` a `/auth/authenticate` usando variables de entorno | EN PROGRESO | BLOQUEO POR CONTRADICCION EXTERNA: mismo gap que API-001; opcional, no bloquea la lectura publica ya verificada. |
| API-003 | Campo real del token verificado | `tools/live-api-auth-probe.mjs`, `js/api.js:authenticate` | Probe exige `token` string no vacio y nunca lo imprime (`token=present`) | EN PROGRESO | BLOQUEO POR CONTRADICCION EXTERNA: mismo gap que API-001; opcional, no bloquea la lectura publica ya verificada. |
