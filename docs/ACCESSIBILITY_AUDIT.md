...
Usa botones disabled, skeleton oculto y fecha activa en `<time datetime>`. `tools/agenda-layout-audit.py` verifica en navegador que dos partidos simultaneos se presentan como columnas visibles, con equipos resueltos y navegacion al siguiente dia.
...
- Resiliencia de API verificada en Playwright: `API_RESILIENCE_AUDIT_PASS retry=429,500 cache_fallback=agenda-cached-data`, con countdown visible y Agenda desde cache ante red bloqueada.
...
- Endpoint injection verificado en Playwright: `ENDPOINT_INJECTION_AUDIT_PASS apiBase=ignored data_host=127.0.0.1:4174 proxy_query=404`, manteniendo test mode local y predecible.
- Headers anti-frame verificados en Playwright: `SECURITY_HEADERS_AUDIT_PASS paths=4 frame_ancestors=none x_frame=DENY`, sin errores de consola al cargar la app local.
