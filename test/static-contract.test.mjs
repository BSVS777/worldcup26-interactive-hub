...
test('README lists the exact package commands and local URLs', async () => {
  ...
  assert.equal(packageJson.scripts['test:live-api'], 'node tools/live-api-probe.mjs');
  assert.equal(packageJson.scripts['test:zoom-reflow'], 'python tools/zoom-reflow-audit.py');
  ...
  assert.match(readme, /`npm run test:live-api`[\s\S]*`node tools\/live-api-probe\.mjs`/);
  assert.match(readme, /`npm run test:zoom-reflow`[\s\S]*`python tools\/zoom-reflow-audit\.py`/);
